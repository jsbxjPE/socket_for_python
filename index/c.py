    ...
    def clisten_start():
        ...
        while True:
            ...
            if '/*stop server*/' == msg_input:
                msg = ("{'a_ip':"+intranet_ip+",'b_ip':"+opposite_ip+",'msg':"+msg_input+",'operating_system':"+operating_system+",'instructions':'stop_server'}")
                c.send(msg.encode('utf-8'))  #发送信息
                instructions.stop_server()
            elif '/*exit*/' == msg_input:
                msg = ("{'a_ip':"+intranet_ip+",'b_ip':"+opposite_ip+",'msg':"+msg_input+",'operating_system':"+operating_system+",'instructions':'user_exit'}")
                c.send(msg.encode('utf-8'))  #发送信息
                instructions.user_exit()
            elif '/*cmd*/' == msg_input:
                msg = ("{'a_ip':"+intranet_ip+",'b_ip':"+opposite_ip+",'msg':"+msg_input+",'operating_system':"+operating_system+",'instructions':'cmd'}")
                c.send(msg.encode('utf-8'))  #发送信息
                instructions.start_cmd()
            elif '/*not cmd*/' == msg_input:
                msg = ("{'a_ip':"+intranet_ip+",'b_ip':"+opposite_ip+",'msg':"+msg_input+",'operating_system':"+operating_system+",'instructions':'not_cmd'}")
                c.send(msg.encode('utf-8'))  #发送信息
                instructions.not_cmd()
            elif '/*download*/' in msg_input:
                file_name = msg_input.split('/*download*/')[1]
            else:
                msg = ("{'a_ip':"+intranet_ip+",'b_ip':"+opposite_ip+",'msg':"+msg_input+",'operating_system':"+operating_system+",'instructions':'None'}")
                c.send(msg.encode('utf-8'))  #发送信息
                ...
                try:
                    ...
                    if '/*run*/' in data:
                        if cmd_open == True:
                            ...
                            c.send("{'a_ip':"+intranet_ip+",'b_ip':"+opposite_ip+",'msg':'msg_clsten didn't open the cmd','operating_system':"+operating_system+",'instructions':'None'}".encode('utf-8'))
